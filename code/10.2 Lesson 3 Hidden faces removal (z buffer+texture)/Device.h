...
public:
    ...
    void init(int width, int height, void* fb) {
        ...
        //bug https://stackoverflow.com/questions/27442885/syntax-error-with-stdnumeric-limitsmax
    ...
    void fillTriangle2(vec3 v1, vec3 v2, vec3 v3, UI32 c);
    void filltriangle_bery(vec3 p1, vec3 p2, vec3 p3, UI32 c);
    void filltriangle_bery_zbuffer(vec3 p1, vec3 p2, vec3 p3, UI32 c);
    void filltriangle_bery_testRGB(vec3 p1, vec3 p2, vec3 p3, vec3 colors[]);
    void filltriangle_bery_texture(vec3 p1, vec3 p2, vec3 p3, vec3 vt[], UI32* texture);
};