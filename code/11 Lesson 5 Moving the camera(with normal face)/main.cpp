...
void render() {
	...
	for (int i = 0; i < model->nfaces(); i++) {
		...
		for (int j = 0; j < 3; j++) {
			...
			// bug
			/*mat4x4 mvp = Matrix_MultiplyMatrix(Projection, ModelView);
			mvp = Matrix_MultiplyMatrix(ViewPort, mvp);
			vec3 screenPoint = Matrix_MultiplyVector(mvp, v);*/
		...
		//device.drawTriangle(screen_coords[0], screen_coords[1], screen_coords[2], 0xff7700);
