...
void filltriangle_bery_testRGB(vec3 p1, vec3 p2, vec3 p3, vec3 colors[]);
void filltriangle_bery_texture(vec3 p1, vec3 p2, vec3 p3, vec3 vt[]);

u32 texture[256][256];
vector<triangle> vtvn_arr{
	// SOUTH
...
void render() {
	...
	vec3 a = { 10 + 0,200 + 0,0 };
	filltriangle_bery_testRGB(a, { a.x + 200,a.y,a.z }, { a.x + 100,a.y + 150,a.z }, cols);
	vec3 cols2[3] = { {1, 1, 0}, {0, 1, 1}, {1, 0, 1} };
	filltriangle_bery_testRGB({ 10,300 + 100,0 }, { 170,300 + 100,0 }, { 90,450 + 100,0 }, cols2);
		...
		for (auto& tri : listTriangles)
		{
			...
			switch (showMode)
			{
				case 0:
					...
					filltriangle_bery_testRGB(tri.p[0], tri.p[1], tri.p[2], cols2);

					//filltriangle_bery_texture(tri.p[0], tri.p[1], tri.p[2], vt2);
					break;
				case 1:
...
vec3 barycentric(vec3 a, vec3 b, vec3 c, vec3 p) {
	...
	*/
	...
	if (std::abs(uv.z) > 1e-2) {
		//printf("uv(%8.2f,%8.2f,%8.2f) p(%8.2f,%8.2f)\n", uv.x, uv.y, uv.z, p.x, p.y);
	...
	for (p.x = bboxMin.x; p.x <= bboxMax.x; p.x++) {
		for (p.y = bboxMin.y; p.y <= bboxMax.y; p.y++) {
			...
			if (p.z > device.zbuffer[(int)p.y][(int)p.x]) {
				...
				u32 c = (int)(col.x * 255) << 16 | (int)(col.y * 255) << 8 | (int)(col.z * 255);
				device_pixel(&device, p.x, p.y, c);
			}
		}
	}
}
